...
class OpenAIClient(VLMClient):
    """
    OpenAI-protocol client, used for both supported backbones.

    base_url is None  -> the OpenAI API, via the Responses endpoint. This is the
                         path that produced all reported results; unchanged.
    base_url is set   -> any OpenAI-compatible server (e.g. a local vLLM serving
                         Qwen3-VL), via chat/completions, which is the endpoint
                         such servers implement.
    """
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gpt-5",
        ...
        if self.base_url and not self.api_key:
            ...
            self.api_key = "EMPTY"
        self.model = model
        kwargs = {"api_key": self.api_key, "timeout": 120.0, "max_retries": 2}
        if self.base_url:
            kwargs["base_url"] = self.base_url
        self.client = openai.OpenAI(**kwargs)

    def _image_data_uri(self, image: Any) -> Tuple[str, str]:
        """Return (media_type, base64_data) for a path, ndarray or raw b64 string."""
        if isinstance(image, str):
            if osp.isfile(image):
                with open(image, "rb") as f:
                    data = base64.b64encode(f.read()).decode("utf-8")
    ...
    def _encode_image(self, image: Any) -> Dict:
        """Encode an image as a Responses-API input_image block."""
        media_type, data = self._image_data_uri(image)
        return {
            "type": "input_image",
            "image_url": f"data:{media_type};base64,{data}"
        }

    def _encode_image_chat(self, image: Any) -> Dict:
        """Encode an image as a chat/completions image_url block."""
        media_type, data = self._image_data_uri(image)
        return {
            "type": "image_url",
    ...
    def query(
        ...
        if self.base_url:
            return self._query_chat(prompt, images, system_prompt,
    ...
    def _query_chat(
        ...
        """chat/completions path — what OpenAI-compatible servers implement."""
        content: List[Dict] = []
        if images:
            for img in images:
                content.append(self._encode_image_chat(img))
        content.append({"type": "text", "text": prompt})
...
# Make the repo root importable (this file is spawned as a script from several
# working directories). Everything else this block used to re-import — osp, io,
# base64, typing, PIL.Image, numpy — is already imported at the top of the file.
import sys
import os
...
def create_vlm_client(provider: str, api_key: Optional[str] = None,
                      model: Optional[str] = None,
                      base_url: Optional[str] = None) -> VLMClient:
    """
    Build the Stage 3 VLM client.

    Both supported backbones speak the OpenAI protocol, so they share one client
    and differ only in (base_url, model):
      openai -> base_url None, the OpenAI API (Responses endpoint)
      qwen   -> base_url of a local OpenAI-compatible server, e.g. vLLM
    """
    provider = provider.lower()

    if provider in ["openai", "gpt4v", "gpt-4v", "gpt4o", "gpt-4o"]:
        return OpenAIClient(api_key=api_key, model=model or "gpt-5.2",
                            base_url=base_url)

    if provider in ["qwen", "qwen3-vl"]:
        if not base_url:
            raise ValueError(
                "The 'qwen' backbone needs an OpenAI-compatible server. Start one, e.g.\n"
                "  vllm serve Qwen/Qwen3-VL-32B-Instruct --port 8000\n"
                "then set vlm.base_url in the config (or $OVAD_VLM_BASE_URL) to "
    ...
    raise ValueError(
        f"Unknown VLM provider: {provider} (supported: openai, qwen)")
...
def main():
    ...
    # Create VLM client. Transport comes from the config's vlm block (with an
    # env override) so Stage 3 uses the same endpoint as Stages 1 and 2.
    ...
    _vlm_cfg = config.get("vlm", {}) if isinstance(config, dict) else {}
    _backbone = os.environ.get("OVAD_VLM_BACKBONE") or args.vlm
    _base_url = os.environ.get("OVAD_VLM_BASE_URL") or None
    _model = os.environ.get("OVAD_VLM_MODEL") or None
    if not os.environ.get("OVAD_VLM_BACKBONE"):
        if _backbone == "qwen":
            _base_url = _base_url or _vlm_cfg.get("base_url")
        else:
            _base_url = None
        _model = _model or (_vlm_cfg.get("openai_model") if _backbone == "openai" else None)
    logger.info(f"[VLM] stage3 backbone={_backbone} model={_model or 'default'} "
                f"endpoint={_base_url or 'the OpenAI API'}")

    try:
        vlm_client = create_vlm_client(_backbone, model=_model, base_url=_base_url)
    except Exception as e:
