...
def main():
    ...
    parser.add_argument("--vlm", default="openai", choices=["openai", "qwen"],
                        help="VLM backbone, forwarded to prompt_vlm.py")
    ...
    cmd = [
        sys.executable, "TubeletGraph/entity_segmentation/cropformer.py",
        "-c", args.config,
        "-d", args.dataset,
        "-s", args.split,
        "--num_workers", str(args.num_workers),
        "--mask_frame_id", str(args.mask_frame_id)  # Added here
    ]
    run_parallel(cmd, "Computing entities (region proposals)", args.gpus)
    
    # Step 2: Computing tubelets
    cmd = [
        sys.executable, "TubeletGraph/tubelet/compute_tubelets_sam.py",
        "-c", args.config,
        "-d", args.dataset,
        "-s", args.split,
        "--num_workers", str(args.num_workers),
    ...
    tubelet_name = f"tubelets_{args.dataset}_cropformer"    
    cmd = [
        sys.executable, "TubeletGraph/semantic_sim/compute_sim_fcclip.py",
        "-c", args.config,
        "-d", args.dataset,
        "-s", args.split,
        "-t", tubelet_name,
    ...
    cmd = [
        sys.executable, "TubeletGraph/get_prediction.py",
        "-c", args.config,
        "-d", args.dataset,
        "-s", args.split,
        "-m", args.method,
        "--mask_frame_id", str(args.mask_frame_id)   # <--- ADDED HERE FOR STEP 4!
    ]
    ...
    cmd = [
        sys.executable, "TubeletGraph/vlm/prompt_vlm.py",
        "-c", args.config,
        "-p", pred_name,
        "--mask_frame_id", str(args.mask_frame_id),   # <--- Make sure this is in run.py!
        "--hint_str", args.hint_str,
        "--vlm", args.vlm,
    ]
