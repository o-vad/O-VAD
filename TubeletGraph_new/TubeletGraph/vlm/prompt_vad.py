...
# SYSTEM_PROMPT_ANOMALY = """You are an expert anomaly detection system for industrial processes.
# Your role is to:
# 1. Carefully observe object states and changes over time through caption and frames
...
PROMPT_ANOMALY_REASONING = """You are analyzing video of a manipulation task for anomalies.
Anomalies should cause or reflect real flaw in object, real flaw in pipeline or real damage to object.
...
The anomalies should be things related to object malfunction, irreversible damage 
to object or industrial pipeline, others like object changes in location and slight
changes in shape that are probably because of noise should be ignored (do NOT include
in your output). For reference, here are some EXAMPLES of anomaly types seen in 
industrial settings, but you should create your own labels if none of these fit:
  ...
  - process_anomaly (e.g., wrong sequence)
...
INSTRUCTION:
Based on the video caption and frames, verify if this CLAIMED ANOMALY really exists following these steps strictly:
1. Check Video Caption, do you find evidence of the claimed anomaly in caption or frames? Do NOT analyze the consistency of caption and frames.
2. Is the description accurate according to either caption or frames?
