...
def meta_update(model, model_weights, meta_init_grads, model_alpha, meta_alpha_grads, 
    ...
    ## To be modified
    # 检查 `meta_init_grads` 是否包含有效的 Tensor
    if not all(isinstance(grad, torch.Tensor) for grad in meta_init_grads):
        raise TypeError("meta_init_grads must be a list or tuple of torch.Tensor objects")

    # 检查 `meta_alpha_grads` 是否包含有效的 Tensor
    if not all(isinstance(grad, torch.Tensor) for grad in meta_alpha_grads):
        raise TypeError("meta_alpha_grads must be a list or tuple of torch.Tensor objects")

    # 更新模型参数 (meta_init)
    with torch.no_grad():
        for param, grad in zip(model.parameters(), meta_init_grads):
            if grad is not None:
                # 确保 `meta_init_optimizer.param_groups[0]['lr']` 是一个标量 (float)
                lr = meta_init_optimizer.param_groups[0]['lr']
                param.copy_(param - lr * grad)

    # 更新学习率 (model_alpha)
    with torch.no_grad():
        for alpha, grad in zip(model_alpha, meta_alpha_grads):
            if grad is not None:
                lr = meta_alpha_optimizer.param_groups[0]['lr']
                alpha.copy_(alpha - lr * grad)
