...
from options.MemAE.training_options import TrainOptions
# import utils
import time
from models.MemAE import AutoEncoderCov3D, AutoEncoderCov3DMem, EntropyLossEncap
...
saving_model_path = os.path.join(saving_root, 'model_' + obj + '/')
...
###### data
video_dataset = data.VideoDataset(tr_data_idx_dir, tr_data_frame_dir, obj=obj, transform=frame_trans)
...
for epoch_idx in range(0, max_epoch_num):
    for batch_idx, (item, frames) in enumerate(tqdm(tr_data_loader)):
