...
from src.VadCLIP.test import test
from dataset.vadclip_dataset import *
from utils.vadclip_utils import get_prompt_text, get_batch_label
...
def train(model, train_loader, test_loader, args, label_map: dict, device):
    ...
    if args.use_checkpoint == True:
        ...
        auc_best = checkpoint['auc'] if 'auc' in checkpoint.keys() else 0
    ...
    for e in tqdm(range(args.max_epoch)):
        ...
        AUC, AP, mAP = test(model, test_loader, args.visual_length, prompt_text, device)
...
if __name__ == '__main__':
    ...
    label_map =  dict({'A': 'normally functioning', 'B': 'fail to funtion'})

    # train_dataset = XDDataset(args.visual_length, args.train_list, False, label_map)
    train_dataset = DynaDataset(args.visual_length, args.train_list, False, label_map)
