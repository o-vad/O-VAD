...
from utils.S3R_utilities.utils import process_feat
...
class Dataset(data.Dataset):
    def __init__(
        ...
        self.data_path_formatter = data_root.joinpath(dataset).joinpath(backbone).joinpath(self.subset).joinpath('{video_id}.npy')
    ...
    def __getitem__(self, index):
        ...
        if 'anomaly_free' in video_id or 'train' in video_id:
        ...
        if self.test_mode:
            ...
            features = np.expand_dims(cv2.resize(features, (width, height), interpolation=cv2.INTER_LINEAR), axis=-1) # CxTxN
