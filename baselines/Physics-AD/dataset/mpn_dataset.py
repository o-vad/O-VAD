    ...
    def setup(self):
        ...
        if "UCF" in self.dir:
            ...
            for video in sorted(videos):
                ...
                # self.videos[video_name] = {}
                # self.videos[video_name]['path'] = video
                # self.videos[video_name]['frame'] = glob.glob(os.path.join(video, 'img_*.png'))
                # self.videos[video_name]['frame'].sort()
                # self.videos[video_name]['length'] = len(self.videos[video_name]['frame'])
            # import pdb;pdb.set_trace()
            # fw = open(self.pkl, "wb")
...
class DynaDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        video_name = self.samples[index].split('/')[-2]
        frame_name = int(self.samples[index].split('/')[-1].split('.')[-2][-4:])
        
        batch = []
        for i in range(self._time_step+self._num_pred):
            if frame_name+i < self.videos[video_name]['length']:
                image = np_load_frame(self.videos[video_name]['frame'][frame_name+i], self._resize_height, self._resize_width)
            else:
                image = np_load_frame(self.videos[video_name]['frame'][-1], self._resize_height, self._resize_width)
            if self.transform is not None:
                batch.append(self.transform(image))

        return np.concatenate(batch, axis=0), self.videos[video_name]['label']
