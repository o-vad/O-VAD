        ...
        if self.transform:
            if c==3:
                frames = torch.cat([self.transform(
                    np.mean(io.imread(os.path.join(v_dir, ('frame_%04d' % i) + '.png')), axis=-1, keepdims=True)).unsqueeze(1) for i
                                    in frame_idx], 1).float()
            elif c==1:
                frames = torch.cat([self.transform(
                    np.expand_dims(io.imread(os.path.join(v_dir, ('frame_%04d' % i) + '.png')), axis=2)).unsqueeze(1) for i
                                    in frame_idx], 1)
        else:
            tmp_frame_trans = transforms.ToTensor() # trans Tensor
            if c==3:
                frames = torch.cat([tmp_frame_trans(
                    io.imread(os.path.join(v_dir, ('frame_%04d' % i) + '.png'))).unsqueeze(1) for i
                                    in frame_idx], 1)
            elif c==1:
                frames = torch.cat([tmp_frame_trans(
                    np.expand_dims(io.imread(os.path.join(v_dir, ('frame_%04d' % i) + '.png'), axis=2))).unsqueeze(1) for i
...
class VideoDatasetOneDir(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.transform:
            frames = torch.cat([self.transform(
                np.mean(io.imread(os.path.join(v_dir, ('frame_%04d' % i) + '.png')).reshape(h, w, c), axis=-1, keepdims=True)).resize_(1, 1, h, w) for i
                                in frame_idx], 1).float()
        else:
            tmp_frame_trans = transforms.ToTensor() # trans Tensor
            frames = torch.cat([self.transform(
