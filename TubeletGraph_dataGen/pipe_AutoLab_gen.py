...
Local scratch is wiped between corrective retries and after NVMe storage is confirmed.
...
IOU for AutoLab:
  context field is [start_frame, end_frame] (two ints) for abnormal videos,
  or [] / absent for normal ones.
  answer field is "Normal" or "Abnormal".
...
class STVADFramework:
    ...
    # NOTE: This method is intentionally unchanged from the original AutoLab
    #       pipeline.  Do NOT modify it.  The only addition is forwarding
    #       hint_str to generate_caption_and_fps(), which now accepts it.
    # ------------------------------------------------------------------
    ...
    def stage1_object_grounding(
        ...
        Stage 1 logic (object detection, bbox scan, SAM3 segmentation, mask
        naming, bbox visualisation) is identical to the original AutoLab
        pipeline.  hint_str is forwarded to generate_caption_and_fps() so the
        caption call can use the secret-mission prompt when a hint is available.
        """
        if self.verbose:
        ...
        try:
            ...
            # hint_str forwarded here — generate_caption_and_fps now accepts it
            ...
            return (str(local_frames_dir), str(mask_path), True,
                    keep_indices, caption, dynamic_fps, best_frame_idx)
    ...
    def stage3_anomaly_detection(
        ...
        hint_str is forwarded to prompt_vad.py via --hint_str when non-empty.
        """
        if self.verbose:
        ...
        # Write to a local temp dir — caller persists to NVMe after IOU check.
        ...
        # Search for the report in the local output dir.
        candidates = [
        ...
        # Report is now in memory — clean up the temp dir immediately.
    ...
    # ------------------------------------------------------------------
    # IOU computation (AutoLab-specific)
    # ------------------------------------------------------------------

    @staticmethod
    def _compute_iou(report: Dict, vqa_metadata: Dict) -> float:
        label            = vqa_metadata.get("answer", "Normal").lower()
        anomaly_detected = report.get("anomaly_detected", False)

        if label == "normal":
            return 1.0 if not anomaly_detected else 0.0
        ...
        for anomaly in report.get("anomalies", []):
            s = anomaly.get("start_frame", None)
            e = anomaly.get("end_frame",   None)
            if s is None or e is None:
                continue
            pred_start = s if pred_start is None else min(pred_start, s)
            pred_end   = e if pred_end   is None else max(pred_end,   e)

        if pred_start is None or pred_end is None:
            return 0.0

        # Frame-level IOU over the union of gt and pred intervals
    ...
    def process_video(
        ...
        """
        Full three-stage pipeline for one AutoLab video.
        ...
        vqa_metadata = (vqa_metadata.get(vp)
                        ...
                        or {})
        vqa_metadata["order"] = "fourth"                # manual sample answer
        ...
        if hint:
            ...
            if video_level_hint.lower() == "abnormal":
                context = vqa_metadata.get("context", [])
                order = vqa_metadata.get("order", "")
                if context and len(context) >= 2:
                    frame_level_hint = f"{int(context[0])}-{int(context[1])}"
                else:
                    frame_level_hint = "unknown"
                hint_str = (
                    f"The video is {video_level_hint}, "
                    f"with abnormal frames [{frame_level_hint}], "
                    f"the anomaly reason is the {order} pipette is experiencing a failure in its aspiration, retention, or dispensing functions."
                )
            else:
                hint_str = f"The video is {video_level_hint}, all frames are normal, all pipettes are functioning correctly."
        ...
        for iteration in range(MAX_CORRECTIVE_RETRIES):
            ...
            # ── IOU VALIDATION ─────────────────────────────────────────────
            iou = self._compute_iou(report, vqa_metadata)

            if self.verbose:
                tqdm.write(f"  [IOU] iteration={iteration+1}  IOU={iou:.4f}  "
                           f"(threshold=0.7)")
            ...
            # ── IOU passed: persist to NVMe and stop ──────────────────────
            if iou > 0.7:
                with suppress_output(self.verbose):
                    # Stage 2 → NVMe
                    ...
                    rpt_path = _report_nvme_path(vp, vqa_metadata, self.dataset_root)
            ...
            # ── IOU failed: clean up and retry ────────────────────────────
            if self.verbose:
                tqdm.write(f"  [IOU] IOU={iou:.4f} < 0.7 — cleaning up and retrying …")

            # Skip cleanup on the last iteration so that best_src_pred_path
            # is still on disk for the fallback NVMe write below.
        ...
        if self.verbose:
            tqdm.write(f"  [IOU] Exhausted {MAX_CORRECTIVE_RETRIES} corrective retries. "
        ...
        return {
            "success":          True,   # pipeline ran; IOU may be below threshold
            "video_path":       vp,
