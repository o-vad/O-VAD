...
def make_config(args, cfg, verbose=True):
    new_cfg = SimpleNamespace(
        ...
        split = args.split,
        outdir = cfg.paths.outdir,      # Using cfg instead of global my_cfg for safety
        intermdir = cfg.paths.intermdir,
    )
    
    # INJECT THE FRAME ID INTO THE METHOD CONFIG
    new_cfg.method.mask_frame_id = args.mask_frame_id
