...
def propagate(sam2, all_tracks, start_frame, clean_prev_memory=20, reverse=False):
    obj_ids = [x for x in sam2.inference_state['obj_idx_to_id'].values()]
    if len(obj_ids) == 0:
        print('No objects to track')
        return
        
    direction = "start <- frame" if reverse else "frame -> end"
    print('Tracking {} objects from id={} to id={} [{}: {}]...'.format(
        len(obj_ids), min(obj_ids), max(obj_ids), direction, start_frame))
        
    for out_frame_idx, out_obj_ids, out_mask_logits in sam2.predictor.propagate_in_video(sam2.inference_state, reverse=reverse):
        # Skip frames we've already initialized or moving the wrong way
        if not reverse and out_frame_idx <= start_frame:
            continue
        if reverse and out_frame_idx >= start_frame:
            continue
        ...
        # Adjust memory cleanup for tracking direction
        delete_f_idx = out_frame_idx + clean_prev_memory if reverse else out_frame_idx - clean_prev_memory
...
def compute_save_init_tubes(cfg, instance_name, save_path, mask_frame_id, max_obj_num=150):
    ...
    init_entity = entity[str(mask_frame_id)]
    tracked_objs = dict()
    all_tracks = {k: dict() for k in range(num_frames)}
    num_it = len(init_entity.items()) // max_obj_num + 1
    pix_thrd = entity[str(mask_frame_id)]['0']['size'][0] * entity[str(mask_frame_id)]['0']['size'][1] * cfg.init_entity_thrd
    ...
    for it in range(num_it):
        ...
        for mask_idx, mask_rle in list(init_entity.items())[start:end]:
            if MaskUtils.area(mask_rle) < pix_thrd:
                ...
                continue
            add_mask(sam2, int(mask_idx), rle_to_bmask(mask_rle), all_tracks, tracked_objs, frame_idx=mask_frame_id)
            
        # Track forwards
        propagate(sam2, all_tracks, start_frame=mask_frame_id, reverse=False)
        # Track backwards if not starting at frame 0
        if mask_frame_id > 0:
            sam2.predictor.reset_state(sam2.inference_state)
            for mask_idx, mask_rle in list(init_entity.items())[start:end]:
                if MaskUtils.area(mask_rle) < pix_thrd:
            ...
            propagate(sam2, all_tracks, start_frame=mask_frame_id, reverse=True)

    dim = entity[str(mask_frame_id)]['0']['size']
    save_all_to_json({'all_tracks': all_tracks, 'tracked_objs': tracked_objs}, save_path, dim=dim)
    sam2.clear_all_cache()
    torch.cuda.empty_cache()


def resolve_init_entity_prompt_conflicts(cfg, sam2, all_tracks, tracked_objs, prompt_mask, mask_frame_id):
    ...
    if len(require_updates) > 0:
        ...
        for obj_idx, obj in require_updates.items():
            ...
            add_mask(sam2, int(obj_idx), obj_mask, all_tracks, tracked_objs, frame_idx=mask_frame_id)
...
def compute_pred(cfg, prompt_mask, instance_name, precomp_mm_path, mask_frame_id):
    ...
    output = sam2_mm.track(mask=rle_to_bmask(prompt_mask), frame_idx=mask_frame_id)
...
def compute_save_all_tubes(cfg, sam2, dataset_name, entity, all_tracks, tracked_objs, prompt_mask, final_all_tracks_path, mask_frame_id):
    ...
    with open(precomp_mm_path, 'r') as f:
        ...
        for frame_idx in all_tracks.keys():
            # FIX: Only load valid forward-tracked masks, ignore the empty `{}` backward gaps
            if frame_idx in prediction and prediction[frame_idx]:
                all_tracks[frame_idx][prompt_obj_id] = prediction[frame_idx]
                
    tracked_objs[prompt_obj_id] = {'mask': prediction.get(mask_frame_id, prompt_mask), 'init_frame_idx': mask_frame_id}

    # =====================================================================
    # FIX: Explicitly track the MAIN prompt object backwards to fill early frames!
    # =====================================================================
    if mask_frame_id > 0:
        print(f"Tracking main prompt object {prompt_obj_id} backwards from frame {mask_frame_id}...")
        sam2.predictor.reset_state(sam2.inference_state)
        add_mask(sam2, prompt_obj_id, rle_to_bmask(prompt_mask), all_tracks, tracked_objs, frame_idx=mask_frame_id)
        propagate(sam2, all_tracks, start_frame=mask_frame_id, reverse=True)
    # =====================================================================
    ...
    for collect_start_frame in range(spacing, num_frames+spacing, spacing):
        ...
        for start_frame in range(scan_start, scan_end):
            ...
            for entity_idx in entity_to_add:
                ...
                # Safely check if the multi_masks exist for this frame
                if start_frame in multi_masks and 1 in multi_masks[start_frame] and 2 in multi_masks[start_frame]:
                    ...
                    # If the prompt wasn't tracked to this frame, overlap is 0
                    tracked_objs[obj_id]['mm_iou'] = 0.0
                    tracked_objs[obj_id]['mm_cover'] = 0.0
        ...
        propagate(sam2, all_tracks, start_frame=collect_start_frame, reverse=False)
...
if __name__ == "__main__":
    ...
    for instance_idx, instance_name in enumerate(instance_names):
        ...
        compute_save_init_tubes(cfg, instance_name, init_all_tracks_path, args.mask_frame_id)
        ...
        for obj_idx, prompt_mask in prompt_objs.items():
            ...
            if not osp.exists(precomp_mm_path):
                compute_pred(cfg, prompt_mask, instance_name, precomp_mm_path, args.mask_frame_id)
            ...
            resolve_init_entity_prompt_conflicts(cfg, sam2, all_tracks, tracked_objs, prompt_mask, args.mask_frame_id)

            compute_save_all_tubes(cfg, sam2, args.dataset, entity, all_tracks, tracked_objs, prompt_mask, final_all_tracks_path, args.mask_frame_id)
