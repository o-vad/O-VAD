...
if __name__ == "__main__":
    ...
    for anno_fname, video_name in instance_names:
        ...
        with open(osp.join(args.tubelet_dir, anno_fname), 'r') as f:
            ...
            init_tracked_objs = [obj_idx for obj_idx, obj_info in tracked_objs.items() if obj_info['init_frame_idx'] == args.mask_frame_id]
            
            # CACHE MISMATCH SAFEGUARD
            if not init_tracked_objs:
                available_inits = [obj_info['init_frame_idx'] for obj_info in tracked_objs.values()]
                print(f"\n[WARNING] Cache mismatch detected for {anno_fname}!")
                print(f"Looking for objects at mask_frame_id={args.mask_frame_id}, but JSON has initialization frames: {list(set(available_inits))}")
                print(f"Falling back to use the earliest available frame in the JSON to prevent a crash.")
                print(f"RECOMMENDATION: Delete your intermediate tubelet caches to fix this properly.\n")
                
                # Fallback to whatever the minimum frame ID in the JSON is
                fallback_frame_id = min(available_inits) if available_inits else 0
                init_tracked_objs = [obj_idx for obj_idx, obj_info in tracked_objs.items() if obj_info['init_frame_idx'] == fallback_frame_id]
                later_tracked_objs = [obj_idx for obj_idx, obj_info in tracked_objs.items() if obj_info['init_frame_idx'] != fallback_frame_id]
            else:
                later_tracked_objs = [obj_idx for obj_idx, obj_info in tracked_objs.items() if obj_info['init_frame_idx'] != args.mask_frame_id]
            
            # Now prompt_obj can safely be extracted
        ...
        if len(candidate_objs) > 0:
            ...
            for ii, path in tqdm.tqdm(enumerate(frame_paths), desc=anno_fname.replace('.json', ''), total=len(frame_paths)):
                ...
                # If there are no target objects in this frame, skip FC-CLIP processing
                if len(obj_ind_str) == 0:
                    out[str(ii)+'_cnts'] = {}
                    out[str(ii)] = {}
                    continue
            ...
            # Chronologically sort the frames
            sorted_frames_int = sorted([int(x) for x in all_tracks.keys()])
            sorted_frames_str = [str(x) for x in sorted_frames_int]
            placeholder = torch.ones(1, 768) * torch.inf
            
            # Fetch the prompt object's clip features in proper temporal order
            query_clip_feat = torch.cat([out.get(f, {}).get(prompt_obj, placeholder) for f in sorted_frames_str])
            query_valid = query_clip_feat[:, 0] < torch.inf
        
            query_clip_feat = F.normalize(query_clip_feat, dim=-1).T
            for obj_idx in candidate_objs:
                # Grab the candidate object's features chronologically
                later_clip_feat_list = [out.get(f, {}).get(obj_idx) for f in sorted_frames_str if obj_idx in out.get(f, {})]
                ...
                if len(later_clip_feat_list) > 0:
                    ...
                    # Slicing up to init_idx correctly grabs the temporally prior frames
