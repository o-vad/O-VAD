...
def get_parser():
    parser = argparse.ArgumentParser(description="Running CropFormer to obtain initial region proposals.")
    ...
    # NEW ARGUMENT: Added so the wrapper script doesn't cause an "unrecognized argument" crash
    parser.add_argument('--mask_frame_id', type=int, default=0, help='Frame ID for the mask input. (Note: CropFormer processes all frames regardless)')
...
if __name__ == "__main__":
    ...
    for vid_dir in vid_dirs:
        ...
        for path in tqdm.tqdm(frame_paths, desc=vid_dir):
            ...
            if len(unique_mask_id) > 0 and unique_mask_id[0] == 0:
