...
class IPADFramework:
    ...
    def process_sample(
        ...
        A known-normal reference video is resolved automatically from the
        dataset layout:  .../IPAD_dataset/XXX/training/frames/01
        Its path is forwarded to Stage 3 only. No caption is generated for
        the reference video. Stages 1 and 2 are unchanged.
        """
        vp = str(seq_dir)
        MAX_RETRIES = 100
        ...
        # ── Resolve normal reference video (Stage 3 only) ──────────
        normal_video_path = _resolve_normal_video_path(vp)
        print("Reference Video:", normal_video_path)

        if self.verbose:
            if normal_video_path:
                print(f"  [Normal] Reference video: {normal_video_path}")
            else:
                print(f"  [Normal] No reference video found for: {vp}")
