...
Where (x1, y1) is the top-left corner and (x2, y2) is the bottom-right corner."""
    ...
    def _detect_with_claude(self, image_path: str) -> List[str]:
        ...
        os.environ["AWS_BEARER_TOKEN_BEDROCK"] = "***REMOVED-CREDENTIAL***"
        aws_client = boto3.client("bedrock-runtime", region_name="us-east-1")
        ...
        # Bedrock converse content block: image first, then the text prompt.
        # The image format is the subtype of the media_type (e.g. "jpeg", "png").
        content = [
            {
                "image": {
                    "format": media_type.split("/")[-1],
                    "source": {"bytes": base64.standard_b64decode(image_data)},
                }
            },
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response:
                    ...
                    # Validate: _parse_object_list must return a non-empty list.
                    objects = self._parse_object_list(response_text)
                    if objects:
                        return objects
                    else:
                        print(f"  Attempt {attempt+1}: output parsing yielded no objects. Retrying...")

            except Exception as e:
                print(f"  Attempt {attempt+1}: API error - {e}")
                time.sleep(5)

        print("  Failed to get a valid object list after maximum retries.")
        return []
    ...
    # ------------------------------------------------------------------
    # Bounding-box localisation of a repeated object class
    # ------------------------------------------------------------------
    ...
    def locate_group_bbox(
        ...
        *object_desc* in the image.
        ...
        if self.vlm_type == "claude":
            raw = self._bbox_with_claude(image_path, prompt)
        elif self.vlm_type == "openai":
            raw = self._bbox_with_openai(image_path, prompt)
        elif self.vlm_type == "ollama":
            raw = self._bbox_with_ollama(image_path, prompt)
        else:
            raise ValueError(f"Unknown VLM type: {self.vlm_type}")
    ...
    def _parse_bbox(
        ...
        try:
            ...
            if match:
                ...
                # FIXED: Map the 0-100 normalized grid back to the original image dimensions
                x1 = int(float(data["x1"]) * img_w / 100.0)
                y1 = int(float(data["y1"]) * img_h / 100.0)
                x2 = int(float(data["x2"]) * img_w / 100.0)
                y2 = int(float(data["y2"]) * img_h / 100.0)
    ...
    def _bbox_with_claude(self, image_path: str, prompt: str) -> str:
        ### QI LONG PRIVACY ###
        import boto3
        import time
        from botocore.exceptions import ClientError
    ...
    def _bbox_with_ollama(self, image_path: str, prompt: str) -> str:
        import requests
        image_data = encode_image_base64(image_path)
        response = requests.post(
            "http://localhost:11434/api/generate",
