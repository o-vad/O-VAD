...
Visualization is intentionally omitted: its output is not consumed by any
downstream stage and adds significant per-object subprocess overhead.
...
def get_parser():
    ...
    parser.add_argument("--vlm_model", default="openai", choices=["openai", "qwen"],
                        help="VLM backbone, forwarded to TubeletGraph/run.py")
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="stvad_fps_stage_") as staging_dir:
        ...
        cmd = [
            sys.executable, "TubeletGraph/run.py",
